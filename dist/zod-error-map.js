...
function fieldLabel(path) {
    const segment = [...path].reverse().find((p) => typeof p === 'string');
    if (!segment)
        return 'This field';
    ...
    switch (issue.code) {
        ...
        case zod_1.ZodIssueCode.invalid_string:
            ...
            if (issue.validation === 'url')
                return { message: `${label} must be a valid URL.` };
            return { message: `${label} is invalid.` };
        ...
        case zod_1.ZodIssueCode.too_big:
            if (issue.type === 'string')
                return { message: `${label} must be ${issue.maximum} characters or fewer.` };
            if (issue.type === 'number')
                return { message: `${label} must be ${issue.maximum} or less.` };
            if (issue.type === 'array')
                return { message: `You can add at most ${issue.maximum} items.` };
            break;
...
zod_1.z.setErrorMap(friendlyErrorMap);
